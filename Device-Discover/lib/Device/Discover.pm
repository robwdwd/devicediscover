...
sub get_sysdesc {
    ...
    if ($self->{'options'}->{'find_community'} and $self->{'result'}->{'community'}) {
...
sub get_community {
    ...
    $self->_logger('debug', "[Community Discover] Can not find any snmp community on this device") if $self->{'options'}->{'debug'};
...
sub _init {
    ...
    if (defined $p{'os'} and defined $p{'protocol'}) {
        croak
          "Device::Discover, passing the os and protocol arguments just adds overhead to your script, there's nothing to discover if you know these already.";
    }
...
sub _logger {
    ...
    my $message = sprintf("[Device::Discover] %s %s\n", $pre, $msg);
